...
English version of ../../entre/simulate_consciencia_entre.py (same dynamics, translated
...
SCENARIOS = (
    Scenario("one_way", "1. trace: S₁ → O₁", ("S1", "O1"), (Edge("S1", "O1", 0.28),)),
    Scenario(
        "mutual", "2. between: S₁ ↔ O₁", ("S1", "O1"),
        (Edge("S1", "O1", 0.28), Edge("O1", "S1", 0.28)),
    ),
    Scenario(
        "nested", "3. observed between: O₂ ↔ (S₁ ↔ O₁)", ("S1", "O1", "O2"),
        (
            Edge("S1", "O1", 0.28), Edge("O1", "S1", 0.28),
    ...
    Scenario(
        "paired", "4. between of betweens", ("S1", "O1", "S2", "O2"),
        (
            Edge("S1", "O1", 0.28), Edge("O1", "S1", 0.28),
...
for i in sample_ids:
    rows.append({
        "time": round(float(results["one_way"]["t"][i]), 1),
        "one_way": round(float(results["one_way"]["closed_between"][i]), 3),
        "mutual": round(float(results["mutual"]["closed_between"][i]), 3),
        "nested": round(float(results["nested"]["closed_between"][i]), 3),
        "between_of_betweens": round(float(results["paired"]["closed_between"][i]), 3),
    })
(OUT / "consciousness-between-data.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
...
def draw_panel(ax, scenario: Scenario, sample_i: int, trail: int = 0):
    ...
    ax.text(-1.28, -1.21, f"trace {data['trace'][sample_i]:.1f}% · closed between {data['closed_between'][sample_i]:.1f}%",
            color=FG, fontsize=7)
    ax.text(-1.28, -1.34, f"return {data['return_to_observed'][sample_i]:.1f}% · 2nd order {data['second_order_effect'][sample_i]:.1f}%",
...
fig.suptitle("Operational hypothesis: a state is born in the bond, returns, and forms second-order bonds",
...
style_chart(ax1)
ax1.set_title("Closed state of the between: memory × alignment × continuity", color=FG, fontsize=11)
ax1.set_xlabel("time", color=FG); ax1.set_ylabel("relational integration (%)", color=FG)
...
style_chart(ax2)
ax2.set_title("The between of betweens shifts S₁ vs. the mutual-only case", color=FG, fontsize=11)
ax2.set_xlabel("time", color=FG); ax2.set_ylabel("S₁ displacement (%)", color=FG)
ax2.legend(frameon=False, labelcolor=FG, fontsize=8, loc="upper right")
fig.savefig(OUT / "consciousness-emerged-in-the-between.png", facecolor=BG, bbox_inches="tight")
plt.close(fig)
...
fig.suptitle("Consciousness in the between? The test measures formation, return and second order — without creating a new node",
...
def animate(frame_number):
    ...
    between_ax.set_title("closed state of the between", color=FG, fontsize=9)
    between_ax.set_xlabel("time", color=FG, fontsize=8); between_ax.set_ylabel("%", color=FG, fontsize=8)
    ...
    for key in ("nested", "paired"):
        ...
        meta_ax.plot(data["t"][:i+1], data["second_order_effect"][:i+1], color=COLORS[key], lw=1.8,
                     label=results[key]["scenario"].title)
    style_chart(meta_ax)
    meta_ax.set_title("second-order effect on S₁", color=FG, fontsize=9)
    meta_ax.set_xlabel("time", color=FG, fontsize=8); meta_ax.set_ylabel("%", color=FG, fontsize=8)
...
anim.save(OUT / "consciousness-emerged-in-the-between.gif", writer=PillowWriter(fps=18))
plt.close(fig)
...
for scenario in SCENARIOS:
    ...
    print(
        scenario.key,
        f"between_tail={np.mean(data['closed_between'][tail]):.2f}",
        f"between_final={data['closed_between'][-1]:.2f}",
