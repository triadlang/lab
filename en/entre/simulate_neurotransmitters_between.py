...
English version of ../../entre/simulate_neurotransmissores_entre.py (same dynamics,
...
rows = []
for i in sample_ids:
    row = {"time": round(float(results["mutual"]["base"]["t"][i]), 1)}
    for key in ("mutual", "nested", "paired"):
        row[f"{key}_phase"] = round(float(results[key]["chemical"]["phase_shift"][i]), 3)
        row[f"{key}_between"] = round(float(results[key]["chemical"]["between_delta"][i]), 3)
    rows.append(row)
(OUT/"neurotransmitters-between-data.json").write_text(
...
def draw_panel(ax, scenario: Scenario, sample_i: int, trail: int = 0):
    ...
    ax.text(-1.30,-1.19,f"base between {base['closed_between'][sample_i]:.1f}% → chemical {chem['closed_between'][sample_i]:.1f}%",
            color=FG,fontsize=7)
    ax.text(-1.30,-1.33,f"Δ between {chem['between_delta'][sample_i]:+.1f} · S₁ shift {chem['phase_shift'][sample_i]:.1f}%",
...
fig.suptitle("The same relation with and without eight continuous coupled neurotransmitters",
...
ax1.set_title("Change of the between state caused by chemistry",color=FG,fontsize=11)
ax1.set_xlabel("time",color=FG); ax1.set_ylabel("chemical − base (p.p.)",color=FG)
ax2.set_title("How much chemistry shifts the observed S₁ itself",color=FG,fontsize=11)
ax2.set_xlabel("time",color=FG); ax2.set_ylabel("phase separation (%)",color=FG)
for ax in (ax1,ax2): ax.legend(frameon=False,labelcolor=FG,fontsize=8,loc="best")
fig.savefig(OUT/"neurotransmitters-change-in-the-between.png",facecolor=BG,bbox_inches="tight")
plt.close(fig)
...
fig.suptitle("Neurotransmitters in the between — outline: base · dot: chemistry · eight spokes: channels",
...
def animate(frame_number):
    ...
    delta_ax.set_title("Δ between state",color=FG,fontsize=9); phase_ax.set_title("S₁ shift",color=FG,fontsize=9)
    delta_ax.set_xlabel("time",color=FG,fontsize=8); phase_ax.set_xlabel("time",color=FG,fontsize=8)
...
anim.save(OUT/"neurotransmitters-change-in-the-between.gif",writer=PillowWriter(fps=18))
plt.close(fig)
