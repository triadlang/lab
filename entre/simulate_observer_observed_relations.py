...
def draw_phase_panel(ax: plt.Axes, scenario: Scenario, sample_i: int, trail: int = 0) -> None:
    ...
    ax.text(-1.25, -1.24, "diferença O₁–S₁: %.1f%%" % data["gap"][sample_i], color=FG, fontsize=7)
    ax.text(-1.25, -1.38, "retorno em S₁: %.1f%%" % data["backreaction"][sample_i], color=MUTED, fontsize=7)
...
def make_static() -> None:
    ...
    ax.set_title("Diferença instantânea entre observado e observador", color=FG, fontsize=11)
    ax.set_xlabel("tempo", color=FG)
    ax.set_ylabel("separação de fase (%)", color=FG)
    ax.set_ylim(0, 105)
    style_chart(ax)
    ...
    ax.set_title("Quanto observar muda o próprio observado", color=FG, fontsize=11)
    ax.set_xlabel("tempo", color=FG)
    ax.set_ylabel("desvio de S₁ contra o caso sem retorno (%)", color=FG)
    ax.set_ylim(0, 105)
    style_chart(ax)
    ax.legend(frameon=False, labelcolor=FG, fontsize=7)

    fig.suptitle("Observador e observado — uma relação, uma relação mútua, uma relação observada e duas relações acopladas",
                 color=FG, fontsize=15, y=0.985)
    fig.savefig(OUT / "observador-observado-relacoes.png", dpi=170, facecolor=BG, bbox_inches="tight")
    plt.close(fig)
...
def make_animation() -> None:
    ...
    def draw(frame_number: int):
        ...
        ax_gap.set_title("Diferença O₁–S₁", color=FG, fontsize=9)
        ax_gap.set_xlabel("tempo", color=FG, fontsize=8)
        style_chart(ax_gap)
        ...
        ax_back.set_title("Mudança provocada no observado", color=FG, fontsize=9)
        ax_back.set_xlabel("tempo", color=FG, fontsize=8)
        style_chart(ax_back)
        ax_back.legend(frameon=False, labelcolor=FG, fontsize=6)
        fig.suptitle("Observador ↔ observado — t=%.1f" % t_value, color=FG, fontsize=14, y=0.985)
        return []

    animation = FuncAnimation(fig, draw, frames=len(sample_indices), interval=70, blit=False)
    animation.save(OUT / "observador-observado-relacoes.gif", writer=PillowWriter(fps=14), dpi=92)
    plt.close(fig)
...
def write_data() -> None:
    ...
    for t_value in sample_times:
        ...
        rows.append({
            "tempo": round(float(t_axis[i]), 1),
            "mutuo": round(float(results["mutual"]["backreaction"][i]), 3),
            "aninhado": round(float(results["nested"]["backreaction"][i]), 3),
            "dois_pares": round(float(results["paired"]["backreaction"][i]), 3),
        })
    (OUT / "observador-observado-relacoes-dados.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    np.savez_compressed(OUT / "observador-observado-relacoes.npz", **arrays)
