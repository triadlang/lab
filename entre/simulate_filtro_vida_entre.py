...
rows = []
for i in sample_ids:
    row = {"tempo": round(float(results["mutual"]["control"]["t"][i]), 1)}
    for key in ("one_way", "mutual", "nested", "paired"):
        life = results[key]["life"]
        row[f"{key}_fase"] = round(float(life["phase_shift"][i]), 3)
        row[f"{key}_entre"] = round(float(life["between_delta"][i]), 3)
        row[f"{key}_filtro"] = round(float(life["filter_divergence"][i]), 3)
    rows.append(row)
(OUT/"filtro-vida-entre-dados.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
...
def draw_panel(ax, scenario, i, trail=0):
    ...
    ax.text(-1.30,-1.19,f"entre químico {control['closed'][i]:.1f}% → vida {life['closed'][i]:.1f}%",color=FG,fontsize=7)
    ax.text(-1.30,-1.33,f"Δ S₁ {life['phase_shift'][i]:.1f}% · diferença dos filtros {life['filter_divergence'][i]:.1f}",color=MUTED,fontsize=7)
...
fig.suptitle("Memória contínua e filtro de vida: a mesma lei, experiências diferentes",color=FG,fontsize=17,y=.985)
...
ax1.set_title("Quanto histórias diferentes deslocam S₁",color=FG,fontsize=11);ax2.set_title("Quanto os filtros pessoais se tornam diferentes",color=FG,fontsize=11)
ax1.set_xlabel("tempo",color=FG);ax2.set_xlabel("tempo",color=FG);ax1.set_ylabel("separação de fase (%)",color=FG);ax2.set_ylabel("distância entre filtros",color=FG)
for ax in (ax1,ax2):ax.legend(frameon=False,labelcolor=FG,fontsize=7,loc="best")
fig.savefig(OUT/"memoria-filtro-vida-mudanca.png",facecolor=BG,bbox_inches="tight");plt.close(fig)
...
fig.suptitle("Filtro de vida — contorno: só química · ponto: química + memória pessoal",color=FG,fontsize=14,y=.98)
...
def animate(frame):
    ...
    style(shift_ax);style(filter_ax);filter_ax.set_ylim(0,2);shift_ax.set_title("deslocamento de S₁",color=FG,fontsize=9);filter_ax.set_title("diferença entre filtros",color=FG,fontsize=9)
    shift_ax.set_xlabel("tempo",color=FG,fontsize=8);filter_ax.set_xlabel("tempo",color=FG,fontsize=8);shift_ax.set_ylabel("%",color=FG,fontsize=8)
...
anim=FuncAnimation(fig,animate,frames=len(frame_ids),interval=55,blit=False);anim.save(OUT/"memoria-filtro-vida-mudanca.gif",writer=PillowWriter(fps=18));plt.close(fig)
